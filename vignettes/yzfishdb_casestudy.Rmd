...
```{r include=FALSE}
knitr::opts_chunk$set(echo = TRUE, eval = FALSE, collapse = TRUE, comment = "#>", fig.path = "casestudy-")
...
p1 <- ggplot(source_cov_plot_data,
             aes(x = Source, y = n_sequences, fill = seq_type)) +
...
p2 <- ggplot(source_cov_plot_data,
             aes(x = Source, y = n_sequences, fill = seq_type)) +
...
seqtab <- seqtab[!rownames(seqtab) %in%
                   c("ZTPSN21IP180-DYXSLCRHK_R1.fq.gz",
                     "ZTPSN21IP270-LBT_R1.fq.gz"), ]
...
seqtab <- readRDS(asv_path)
seqtab <- seqtab[!rownames(seqtab) %in% c("ZTPSN21IP180-DYXSLCRHK_R1.fq.gz",
                                          "ZTPSN21IP270-LBT_R1.fq.gz"), ]
asv_seqs <- colnames(seqtab)
asv_ids <- sprintf("ASV_%05d", seq_along(asv_seqs))
names(asv_seqs) <- asv_ids
...
A key feature of `regionbarcoder` is the ability to easily benchmark different assignment methods. Here, we compare the dependency-free exact matching method against the robust BLASTn method, using both the full YzFishDB reference database and the 12S subset of the database.
...
# 12S Database subset
t_exact_12 <- system.time({
  res_exact_12 <- rb_assign_edna(
    asv_fasta = asv_fasta,
    con = con,
    marker = "12S",
    method = "exact",
...
# Run BLASTn
t_blast_12 <- system.time({
  res_blast_12 <- rb_assign_edna(
    asv_fasta = asv_fasta,
    con = con,
    marker = "12S",
    method = "blastn",
...
comparison <- data.frame(
  ...
  Success_Rate_Pct = c(round(
    mean(res_exact$assignment_status != "no_match") * 100, 2
  ), round(
    mean(res_blast$assignment_status != "no_match") * 100, 2
  )),
  Database = c("Full", "Full")
)
## 12S database subset
comparison_12 <- data.frame(
  Method = c("Exact", "BLASTn"),
  Time_seconds = c(t_exact_12[3], t_blast_12[3]),
  ASVs_Assigned = c(
    sum(res_exact_12$assignment_status != "no_match"),
    sum(res_blast_12$assignment_status != "no_match")
  ),
  Success_Rate_Pct = c(round(
    mean(res_exact_12$assignment_status != "no_match") * 100, 2
  ), round(
    mean(res_blast_12$assignment_status != "no_match") * 100, 2
  )),
  Database = c("12S", "12S")
)

comparison_df <- rbind(comparison, comparison_12)
...
Comparison between the assigned ASVs also reveal that using the full database assigned `r nrow(output$assign_compare)` additional ASVs. However these assignments were all flagged as low quality matches (`low`), emphasizing the importance of selecting the most appropriate reference database for taxonomic assignment.

```{r}
assign_12 <- filter(res_blast_12, assignment_status == "unique")
assign_full <- filter(res_blast, assignment_status == "unique")

assign_compare_df <- full_join(
  dplyr::select(assign_full, asv_id, species, confidence),
  dplyr::select(assign_12, asv_id, species, confidence),
  by = 'asv_id',
  suffix = c("_f", "_12")
) |>
  mutate(taxa_diff = species_f == species_12,
         conf_diff = confidence_f == confidence_12) |>
  replace_na(list(taxa_diff = FALSE, conf_diff = FALSE)) |>
  filter(!taxa_diff | !conf_diff)
print(dplyr::select(assign_compare_df, -taxa_diff, -conf_diff))
```

```{r eval=TRUE, echo=FALSE}
output$assign_compare |> dplyr::select(-taxa_diff, -conf_diff)
```
...
```{r}
ties <- res_blast_12 |> filter(assignment_status == "check_full_tie")
...
res_blast_12 <- res_blast_12 |>
  mutate(
    species = case_when(
      asv_id %in% c("ASV_07559", "ASV_08335", "ASV_09885") ~ "Gymnodiptychus pachycheilus",
      TRUE ~ species
    ),
    assignment_status = case_when(
      asv_id %in% c("ASV_07559", "ASV_08335", "ASV_09885") ~ "unique",
      TRUE ~ assignment_status
    ),
    tie_taxa = case_when(
      asv_id %in% c("ASV_07559", "ASV_08335", "ASV_09885") ~ NA_character_,
      TRUE ~ tie_taxa
    )
  )
```
...
cat("Number of ASVs requiring manual curation:",
    nrow(output$ties),
    "\n")
...
cat("Final ecological matrix dimensions (Species x Samples):",
    dim(species_sample),
    "\n")
```
...
species_sample <- rb_build_species_matrix(
  assignment = output$res_blast_12,
  asv_table = asv_table,
  include_ambiguous = FALSE, 
  unassigned = FALSE         
)
