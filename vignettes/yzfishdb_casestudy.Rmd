...
seqtab <- seqtab[!rownames(seqtab) %in% c("ZTPSN21IP180-DYXSLCRHK_R1.fq.gz", "ZTPSN21IP270-LBT_R1.fq.gz"),]
...
t_blast <- system.time({
  res_blast <- rb_assign_edna(
    ...
    blastn = blastn_path, makeblastdb = makeblastdb_path
  )
})
...
comparison_12 <- data.frame(
  ...
  Success_Rate_Pct = c(
    round(mean(res_exact_12$assignment_status != "no_match") * 100, 2),
    round(mean(res_blast_12$assignment_status != "no_match") * 100, 2)
  ),
  Database = c("12S", "12S")
)
...
assign_compare_df <- full_join(dplyr::select(assign_full, asv_id, species, confidence), dplyr::select(assign_12, asv_id, species, confidence), by = 'asv_id', suffix = c("_f","_12")) |>
